        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-5 lg:justify-between lg:flex-row">
                {Object.keys(planOptions[initForm.yearly ? 'yearly' : 'monthly']).map((planName) => {
                    return (
                        <div className="w-[150px] h-[185px]" key={planName}>
                            <SelectableContainer selected={initForm.plan === planName} onSelected={() => dispatch(setForm({ ...updatedForm, ...{ plan: planName } }))} content={
                                <div className="flex gap-3 lg:flex-col lg:items-between">
                                    <img className="h-[45px] w-[45px]" src={`/assets/images/${planImages[planName]}`} />
